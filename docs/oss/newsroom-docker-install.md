...
scout auth login --site https://newsroom.example --label "Newsroom terminal"
scout scouts list
```

The newsroom frontend's discovery document supplies its API base and public
gateway key. The browser approval creates a dedicated API key and saves it
without displaying it. The existing `PUBLIC_APP_URL` pins `cli-auth` to the
same public frontend origin; `SCOUTPOST_SITE_URL` is an optional narrower
override.

Use the newsroom deployment URL and MCP URL generated by the self-host install.
Do not point a self-hosted newsroom at `scoutpost.ai`
