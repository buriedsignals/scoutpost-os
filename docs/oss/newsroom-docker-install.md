...
coJournalist self-hosting uses this stack:
...
| End-user CLI | `cojo` | Optional journalist/agent client after deployment |
...
Open `/setup` in the coJournalist app and download the Docker installer bundle.
...
1. Finds or clones the coJournalist OSS checkout.
...
- hosted coJournalist Supabase references accidentally copied into self-host config
...
3. Fetches upstream coJournalist OSS.
...
install the `cojo` CLI onto journalists' machines.

After deployment, install `cojo` per editor or agent machine:

```bash
deno install -A -g -n cojo https://raw.githubusercontent.com/buriedsignals/cojournalist-os/main/cli/main.ts
cojo config set api_url=https://<your-supabase-url>/functions/v1
cojo config set supabase_anon_key=<SUPABASE_ANON_KEY>
cojo config set api_key=cj_...
cojo scouts list
```

Use the newsroom deployment URL, API base, MCP URL, and API keys generated by
the self-host install. Do not point a self-hosted newsroom at `cojournalist.ai`
...
If the frontend points at the hosted coJournalist Supabase project, rerun setup
