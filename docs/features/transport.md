...
AI-adjacent physical-movement monitoring: **vessels** (AIS) and **aircraft**
(ADS-B). A transport scout alerts
...
20 MMSIs / ICAO hexes per scout) is mandatory for all modes.
...
- Cadence: `3h` / `6h` / `12h` / `daily` (default `3h`). No sub-3h cadences.
...
vessel sampler is stale, runs record a visible `skipped`
...
older than 125 minutes are excluded.
