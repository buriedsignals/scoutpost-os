...
- Import-off creation records URL membership of the documents the tracked
  pages list (no parsing, no archive-size cap below 5,000 URLs). A scheduled
  run queues only new URLs, at most two per run, and re-hashes the newest
  three known documents to detect a replaced file at a stable URL.
