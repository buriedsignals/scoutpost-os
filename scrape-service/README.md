...
|---|---|---|
| `POST /scrape` `{url, timeout_ms?}` | Bearer | Playwright render → `ScrapeResult` JSON, including a quality-gated semantic-main projection when available. A browser download signal makes one guarded PDF-parser transition instead of repeating navigation. |
...
Error taxonomy: upstream failure → 502, typed `navigation_timeout` → 504,
...
Downloaded PDFs return the same `ScrapeResult` shape: extracted text in
`markdown`, null HTML fields, and `metadata.document` with PDF type, page count,
character count and parser. This includes extensionless download URLs. The
parser retains redirect/DNS safety, byte limits and a bounded download deadline.
