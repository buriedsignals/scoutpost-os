...
def replay_arrivals(args: argparse.Namespace) -> dict:
    ...
    combined_overage = max(
        0.0,
        existing_outbound + workflow_outbound_gb - PRO_INCLUDED_OUTBOUND_GB,
    )
    incremental_egress_cost = (
        combined_overage - prior_overage
    ) * OUTBOUND_OVERAGE_DOLLARS_PER_GB
    monthly_cost = page_cost + startup_cost + incremental_egress_cost + 2.00
    report = {
        ...
        "monthly_cost_components": {
            ...
            "incremental_egress": round(incremental_egress_cost, 4),
        },
        "outbound": {
            "measured_bytes_per_job": round(outbound_bytes / measured_pages, 3),
            "projected_workflow_gb": round(workflow_outbound_gb, 6),
            "existing_workspace_gb": existing_outbound,
            "pro_included_gb": PRO_INCLUDED_OUTBOUND_GB,
        },
    }
    args.report.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
    return report
