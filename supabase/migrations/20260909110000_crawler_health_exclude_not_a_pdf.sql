-- Crawler workflow health: parse_pdf reporting not_a_pdf is a content sniff,
-- not a failure. docparse.parseDocument() tries the PDF path first for every
-- civic document link; the scrape-service answers 415 for an HTML page (a
-- meeting agenda, a session listing) and the caller immediately falls back to
-- a normal scrape of the same URL. The row is recorded terminal by design.
--
-- Civic scouts probe several meeting pages back to back, so these sniffs
-- cluster (23 on 2026-09-07, 3 inside one minute on 2026-09-09) and were
-- opening crawler_workflow_health incidents on a healthy queue. Exclude them
-- from terminal_failed_recent alongside the anti-bot delegation and the
-- unresolvable-host exclusions.
